...
class LoanDetailScreen extends StatelessWidget {
  ...
  Widget _pageBuilder() {
    return SafeArea(
      child: Scaffold(
          ...
          child: Padding(
            ...
            child: BlocBuilder<LoanDetailBloc, LoanDetailState>(
              builder: (context, state) {
                print(state);
                final loanDetialBloc = BlocProvider.of<LoanDetailBloc>(context);
                    ...
                    child: Column(
                      children: [
                        ...
                        Container(
                          child: Row(
                            children: [
                              _frequnceyFormBuilder(state.list[0].collected),
                              SizedBox(
                                width: 15,
                              ),
                            ],
                        ...
                        _borrowerFormBuilder(state.list[0].person),
                ...
                loanDetialBloc.add(LoansDetailLoad(id));
                return Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    SizedBox(
        ...
        bottomNavigationBar: BlocBuilder<LoanDetailBloc, LoanDetailState>(
          builder: (context, state) {
            final billDetailBloc = BlocProvider.of<LoanDetailBloc>(context);
              ...
              return Padding(
                ...
                child: Row(
                  children: [
                    TextButton(
                      onPressed: () {
                        if (_formKey.currentState!.validate()) {
                          billDetailBloc.add(LoansEditSave(
                            id: id,
                            title: title_value,
                    ...
                    TextButton(
                      onPressed: () {
                        billDetailBloc.add(LoansEditCancel(state.ids));
                      },
                      child: Container(
                        width: 100.0,
                        height: 65.0,
  ...
  Widget _billTitleBuilder() {
    return BlocBuilder<LoanDetailBloc, LoanDetailState>(
      builder: (context, state) {
        final loanDetailBloc = BlocProvider.of<LoanDetailBloc>(context);
        if (state is LoanDetailEditing) {
          return Container(
        ...
        if (state is LoanDetailLoaded) {
          String title = state.loans[0].title;
          return Container(
            margin: EdgeInsets.fromLTRB(22.0, 0.0, 0.0, 5.0),
            child: Row(
              children: [
                ...
                Padding(
                  ...
                  child: IconButton(
                    ...
                    onPressed: () {
                      loanDetailBloc.add(LoansEdit(state.id));
                    },
                  ),
                ),
              ],
            ...
            children: [
              ...
              Padding(
                ...
                child: IconButton(
                  icon: Icon(Icons.edit),
                  onPressed: () {
                    loanDetailBloc.add(LoansEdit(1));
                  },
                ),
              ),
            ],
    ...
    return BlocBuilder<LoanDetailBloc, LoanDetailState>(
      builder: (context, state) {
        if (state is LoanDetailLoaded) {
          ...
          final String startDate = formatter.format(state.loans[0].startDate);
          String description = state.loans[0].description;
          double deposit = state.loans[0].amount;
          String person = state.loans[0].person;
          return Container(
  ...
  Widget _depositeList() {
    return BlocBuilder<LoanDetailBloc, LoanDetailState>(
      builder: (context, state) {
        if (state is LoanDetailLoaded) {
          return _depositsListBuilder(state.loans[0].deposit);
        }
        return Container();
      },
    );
  ...
  Widget _depositsListBuilder(List depositList) {
    ...
    for (var i = 0; i < depositList.length; i++) {
      double amount = depositList[i]['Collected Amount'].toDouble();
      DateTime date = inputFormat.parse(
          depositList[i]['collection Date'][2].toString() +
              '-' +
              depositList[i]['collection Date'][1].toString() +
              '-' +
              depositList[i]['collection Date'][0].toString());
  ...
  Widget _frequnceyFormBuilder(double collceted) {
    return Padding(
  ...
  Widget _borrowerFormBuilder(String person) {
    return Padding(
      padding: const EdgeInsets.fromLTRB(20.0, 10, 0.0, 0),
      child: Card(
        shape: RoundedRectangleBorder(
  ...
  Form _buildBottomSheet(BuildContext context, int id) {
    ...
    final bloc = BlocProvider.of<LoanDetailBloc>(context);
    return Form(
      key: _formDepositKey,
      child: Container(
        height: 500,
